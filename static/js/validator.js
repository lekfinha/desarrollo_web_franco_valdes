...
new MultiSelectTag('sports')
let region_select = document.getElementById("region");
const displayRegion = () => {
    for(const info in regioncomuna["regiones"]) {
        let region_content = regioncomuna["regiones"][info];
        const region_option = document.createElement("option");
        region_option.value = region_content["id"];
        region_option.innerText = region_content["nombre"];
        region_select.appendChild(region_option);
    }
}

const updateComunas = () => {
    const selected_region = document.getElementById("region");
    const selected_id = selected_region.options[selected_region.selectedIndex].value;
    const selected_info = regioncomuna.regiones.find(region => region.id == selected_id);
    const selected_comuna = document.getElementById("comuna");
    selected_comuna.innerHTML = "";
    selected_info.comunas.forEach(comuna => {
        const added_comuna = document.createElement("option");
        added_comuna.value = comuna["id"];
        added_comuna.innerText = comuna["nombre"];
        selected_comuna.appendChild(added_comuna);
    })
}
...
const ValidateTextBox80 = (string) => {
	if(!string) {
		return true;
	}
	if(string.length > 80) {
...
const ValidateSports = (value) => {
    if(value.selectedOptions.length < 1) {
        console.log(value.selectedOptions.length);
        return false;
    }
...
const ValidateForm = () => {
    let myForm = document.forms["artesano-form"];
	let nombre= document.getElementById("nombre").value;
	let email= document.getElementById("email").value;
	let contacto= document.getElementById("telefono").value; 
	let region= document.getElementById("region").value;
	let comuna= document.getElementById("comuna").value;
	let textbox = document.getElementById("descripcion").value;
	let sports = document.getElementById("sports");

    let fotos = [myForm["foto-1"].files, myForm["foto-2"].files, myForm["foto-3"].files];
	...
	if(!ValidateTextBox80(textbox)) {
		setInvalidInput("Descripcion");
	}

    if(!ValidateSports(sports)) {
        setInvalidInput("Tipo de Artesania");
    }
    let foto_type = true;
    let foto_quantity = false;
    for(const foto of fotos) {
        if(foto.length > 0) {
            foto_quantity = true;
            foto_type &&= validateFotos(foto);
        }
    }
    if(!foto_type || !foto_quantity) {
        setInvalidInput("Debe añadir al menos una imagen de su artesania o esta subiendo un archivo que no es una imagen")
    }
    ...
    if (!isValid) {
        ...
        document.getElementById("form-group").hidden = true;
        ...
        confirm_button.addEventListener("click", () => {
            ...
            document.getElementById("return-box").hidden = false;
        });
        deny_button.addEventListener("click", () => {
            document.getElementById("confirm-box").hidden = true;
            document.getElementById("form-group").hidden = false;
        });
    }
}
